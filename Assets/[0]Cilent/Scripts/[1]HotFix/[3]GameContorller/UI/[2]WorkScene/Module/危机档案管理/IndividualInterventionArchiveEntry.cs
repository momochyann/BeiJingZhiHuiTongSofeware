...
public class IndividualInterventionArchiveEntry : MonoBehaviour, IController, IEntry
{
    ...
    public void DisEntry(int index)
    {
        ...
        if (EntryRawValue != null)
        {
            ...
            if (干预实施方案文本框 != null)
            {
                干预实施方案文本框.text = EntryRawValue.FangAnName;
            }
            // 更新生成按钮文本
            if (生成按钮 != null)
            {
                ...
                生成按钮.onClick.AddListener(() =>
                {
                    if (!EntryRawValue.isCreateReport)
                    {
                        ...
                        this.SendCommand(new EditEntryCommand(EntryRawValue, 新条目, "IndividualInterventionArchiveModel"));
    ...
    {
        var model = this.GetModel<IndividualInterventionArchiveModel>();
        var message = model.individualInterventionArchives[index];
