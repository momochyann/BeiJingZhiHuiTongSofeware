...
public class ObjectiveAssessmentArchiveEntry : MonoBehaviour, IController, IEntry
{
    ...
    public void DisEntry(int index)
    {
        ...
        if (EntryRawValue != null)
        {
            ...
            // 量表名称
            if (量表名称 != null && EntryRawValue.scaleName != null)
            {
                量表名称.text = EntryRawValue.scaleName;
            }
            // 更新生成按钮文本
            if (生成按钮 != null)
            {
                ...
                生成按钮.onClick.AddListener(() =>
                {
                    if (!EntryRawValue.isCreateReport)
                    {
                        ...
                        this.SendCommand(new EditEntryCommand(EntryRawValue, 新条目, "ObjectiveAssessmentArchiveModel"));
    ...
    {
        var model = this.GetModel<ObjectiveAssessmentArchiveModel>();
        var message = model.objectiveAssessmentArchives[index];
